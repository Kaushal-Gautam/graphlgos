...
{    
    //enqueue the src to queue
    hold_queue.push_back(src);

    //while queue isnt empty iteration
    while (hold_queue.empty()!=true)
    {
        //dequeue from the queue
        int temp = hold_queue.front();
        hold_queue.pop_front();   // same operation as dequeue
        ...
        for(int i=0; i<num; i++)
        {
            ...
            {
                //if the vertice isnt traversed and isnt in the queue, enqueued 
                if(isTraversed(i)==false && (std::find(hold_queue.begin(), hold_queue.end(), i) == hold_queue.end()))
                {
                    hold_queue.push_back(i);
                }
            }
        }
    }
